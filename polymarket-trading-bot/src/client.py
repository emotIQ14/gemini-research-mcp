...
class ClobClient(ApiClient):
    ...
    def __init__(
        ...
        Args:
            ...
            funder: Safe/Proxy wallet address (used as order owner)
        ...
        # EOA address that signed the L2 auth — used in POLY_ADDRESS header
        self.signer_address = signer_address or funder
    ...
    def _build_headers(
        ...
        if self.api_creds and self.api_creds.is_valid():
            ...
            headers.update({
                "POLY_ADDRESS": self.signer_address,   # EOA, not Safe
    ...
    def post_order(
        ...
        # Build request body — signature va dentro del order
        ...
        _lg.getLogger(__name__).debug(f"ORDER BODY SENT: {body_json}")
