...
class OrderSigner:
    ...
    # Domain para AUTENTICACION (L1/L2 api-key derivation)
    AUTH_DOMAIN = {
        "name": "ClobAuthDomain",
        "version": "1",
        "chainId": 137,
    ...
    # Domain para ORDENES (CTF Exchange contract en Polygon)
    ORDER_DOMAIN = {
    ...
    # Mantener DOMAIN para compatibilidad con sign_auth_message
    DOMAIN = AUTH_DOMAIN
    ...
    def sign_order(self, order: Order) -> Dict[str, Any]:
        ...
        try:
            ...
            # Para signature_type 2 (Gnosis Safe) el nonce debe ser 0
            nonce = 0 if order.signature_type == 2 else order.nonce
            order_message = {
                "salt": salt,
            ...
            # Sign the order with the CTF Exchange domain (NOT auth domain)
            signable = encode_typed_data(
